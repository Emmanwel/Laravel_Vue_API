  ...
  <PageComponent>
    <template v-slot:header>
      <div class="flex justify-between items-center">
        ...
        <div class="flex">
          <input type="text" placeholder="Search" class="border border-gray-400 rounded-l-lg py-2 px-4 w-150"
            v-model="input">
          <button class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-r-md"
            @click="filteredList(survey)">Search</button>
        </div>
    ...
    <div v-else-if="surveys.data.length">
      ...
      <div class="flex justify-center mt-5">
        <nav class="relative z-0 inline-flex justify-center rounded-md shadow-sm -space-x-px" aria-label="Pagination">
          ...
          <a v-for="(link, i) of surveys.links" :key="i" :disabled="!link.url" href="#" @click="getForPage($event, link)"
            aria-current="page"
