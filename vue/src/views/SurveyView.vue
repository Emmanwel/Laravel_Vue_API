    ...
    <form v-else @submit.prevent="saveSurvey" class="animate-fade-in-down">
      <div class="shadow sm:rounded-md sm:overflow-hidden">
        ...
        <div class="px-4 py-5 bg-white space-y-6 sm:p-6">
          ...
          <!-- Title -->
          <div>
            <label for="title" class="block text-sm font-medium text-gray-700">Item Name </label>
...
// Get Item loading state, which only changes when we fetch survey from backend
...
// Watch to current Item  data change and when this happens we update local model
watch(
...
// If the current component is rendered on Item  update route we make a request to fetch survey
if (route.params.id) {
...
function saveSurvey() {
  ...
  store.dispatch("saveSurvey", { ...model.value }).then(({ data }) => {
    store.commit("notify", {
      type: "success",
      message: "The Item was successfully " + action,
    });
    router.push({
      name: "SurveyView",
...
function deleteSurvey() {
  if (
    confirm(
      `Are you sure you want to delete this Item? Operation can't be undone!!`
    )
  ) {
